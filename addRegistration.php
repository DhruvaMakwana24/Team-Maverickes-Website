<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_SESSION['event_srno'])){
        ...
        foreach(array_reverse($data['participants']) as $participant)
        {
            $partsrno = $participant['participant_srno'];
            break;
        }
        
        $dataArray2 = array("participant_srno" => $partsrno, "event_srno"=>$eventSrno);
        $data2 = json_encode($dataArray2);
        postApi('registrations', $data2);

        $response = getApi('registrations');
        $data = json_decode($response, true);
        foreach(array_reverse($data['registrations']) as $registration){
        ...
        $response = getApi('events/'.$eventSrno);
        $data = json_decode($response, true);
        ...
        $subject="Registration Successfull".$_POST['contact'];
        $message = "Dear $participant_name,\n\nhave successfully registered for ".$eventname."\n".$_POST['contact'];
        $header = "FROM: Team Mavericks <mavericksbodhantra@gmail.com>";
        mail("mavericksbodhantra@gmail.com",$subject,$message, $header);
    }
